...
$MESS["HIBLOCK_PROP_DIRECTORY_DESCRIPTION_PLUS"] = "Справочник Plus";
 ...
 */
class CIBlockPropertyDirectoryPlus
{
    ...
    public static function GetUserTypeDescription()
    {
        return array(
            'PROPERTY_TYPE' => 'S',
            'USER_TYPE' => 'directoryplus',
            'DESCRIPTION' => "Справочник Plus   ",
    ...
    public static function GetSettingsHTML($arProperty, $strHTMLControlName, &$arPropertyFields)
    {
        ...
        $settings = CIBlockPropertyDirectoryPlus::PrepareSettings($arProperty);
...
<tr>
	<td colspan="2" style="text-align: center;">
		...
		<input type="hidden" name="{$strHTMLControlName["NAME"]}[LANG][ID]" value="{$headingXmlId}">
...
HIBSELECT;
    ...
    public static function GetPropertyFieldHtml($arProperty, $value, $strHTMLControlName)
    {
        ...
        $options = CIBlockPropertyDirectoryPlus::GetOptionsHtml($arProperty, array($value["VALUE"]));
    ...
    public static function GetPropertyFieldHtmlMulty($arProperty, $value, $strHTMLControlName)
    {
        ...
        {
            $options = CIBlockPropertyDirectoryPlus::GetOptionsHtml($arProperty, $values);
        ...
        else
        {
            ...
            foreach($values as $property_value_id=>$value)
            {
                ...
                $options = CIBlockPropertyDirectoryPlus::GetOptionsHtml($arProperty, array($value));
    ...
    public static function GetOptionsHtml($arProperty, $values)
    {
        ...
        if($highLoadIBTableName != '')
        {
            ...
            //echo "<pre>"; print_r(self::$arFullCache[$highLoadIBTableName]);echo "</pre>";
    ...
    public static function GetExtendedValue($arProperty, $value)
    {
        ...
        if (!isset(self::$arItemCache[$tableName][$value['VALUE']]))
        {
            ...
            if (!empty($arData))
            {
                ...
                if (isset($arData['ID']) && $arData['ID'] == $value['VALUE'])
                {
    ...
    private static function getEntityFieldsByFilter($tableName, $listDescr = array())
    {
        ...
        if (!empty($tableName))
        {
            if (!isset(self::$hlblockCache[$tableName]))
            {
               ...
               // echo "<pre>"; print_r( self::$hlblockCache[$tableName]);echo "</pre>";
            }
            if (!empty(self::$hlblockCache[$tableName]))
            {
                ...
                if(isset(self::$directoryMap[$tableName]['UF_NUMBER']))
                {
                    $listDescr['select'][] = 'UF_NUMBER';
                }
                if ($nameExist)
                ...
                $listDescr['order']['ID'] = 'ASC';
//echo "<pre>"; print_r($listDescr);echo "</pre>";
                $rsData = $entityDataClass::getList($listDescr);
                while($arData = $rsData->fetch())
                {
                    if (!$nameExist)
                        if(isset(self::$directoryMap[$tableName]['UF_NUMBER']))
                            $arData['UF_NAME'] = $arData['UF_NUMBER'];
                            else
                        $arData['UF_NAME'] = $arData['ID'];
...
AddEventHandler("iblock", "OnIBlockPropertyBuildList", array("CIBlockPropertyDirectoryPlus", "GetUserTypeDescription"));
